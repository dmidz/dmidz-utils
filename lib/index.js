...
global.check = function (v, wishType, flag) {
	...
	switch (wt) {
		...
		case 'elements' :
		case 'collection' :
			let min = 1;// default min length required
...
const mod = module.exports = {
	mixin: function (dest, src, deep) {
		...
		if (!check(src, 'o'))  return dest;
		if (check(deep, 'a')) {// copy only some properties
			for (let i = 0, max = deep.length; i < max; i++) {
				let prop = deep[i];
			...
			for (let prop in src) {
				dest[prop] = (deep && src[prop] && src[prop].constructor === Object)
					? mod.mixin(dest[prop], src[prop], deep ) : src[prop];
			}
		}
		return dest;
	}
